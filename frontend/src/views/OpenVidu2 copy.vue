      ...
      <div id="join" class="vertical-center">
        ...
        <div id="join-dialog" class="jumbotron">
          ...
          <div id="login-info">
            ...
            <button id="logout-btn" class="btn btn-warning" @click="logOut()">
              Log out
            </button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    getToken(callback) {
      ...
      this.httpPostRequest(
        "http://localhost:8080/api-sessions/get-token",
        { sessionName: this.sessionName, user: this.user },
    ...
    removeUser() {
      this.httpPostRequest(
        "http://localhost:8080/api-sessions/remove-user",
        { sessionName: this.sessionName, token: this.token },
        "User couldn't be removed from session",
        (response) => {
          console.log(response);
          console.warn(
            "You have been removed from session " + this.sessionName
          );
        }
      );
    },
    ...
    initMainVideoThumbnail() {
      $("#main-video video").css(
        "background",
        "url('../assets/images/subscriber-msg.jpg') round"
      );
    },
